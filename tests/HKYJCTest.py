...
annotate_dates([tree])
acr_result_f81 = acr(tree, pd.read_csv(STATES_INPUT, index_col=0, header=0, sep='\t')[[feature]],
                     prediction_method=MPPA, model=JC)[0][0]

df = pd.read_csv(STATES_INPUT, index_col=0, header=0, sep='\t')[[feature]]
tree = read_tree(TREE_NWK)
annotate_dates([tree])
acr_result_hky_free_kappa = acr(tree, df, prediction_method=MPPA, model=HKY,
                                column2parameters={feature: {'A': .25, 'T': .25, 'C': .25, 'G': .25}})[0][0]
tree = read_tree(TREE_NWK)
annotate_dates([tree])
acr_result_hky_free_freqs = acr(tree, df, prediction_method=MPPA, model=HKY,
                                column2parameters={feature: {KAPPA: 1}})[0][0]
tree = read_tree(TREE_NWK)
annotate_dates([tree])
...
acr_result_hky = acr(tree, df, prediction_method=MPPA, model=HKY,
                     column2parameters={feature: {KAPPA: 1, 'A': .25, 'T': .25, 'C': .25, 'G': .25}})[0][0]
