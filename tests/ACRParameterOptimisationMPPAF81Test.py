...
annotate_dates([tree])
acr_result = acr(tree, df, prediction_method=MPPA, model=F81)[0][0]
...
class ACRParameterOptimisationMPPAF81Test(unittest.TestCase):
    ...
    def test_rerooted_values_are_the_same(self):
        for _ in range(5):
            ...
            rerooted_acr_result = acr(rerooted_tree, df, prediction_method=MPPA, model=F81)[0][0]
    ...
    def test_changes_per_avg_branch(self):
        self.assertAlmostEqual(0.107, acr_result[CHANGES_PER_AVG_BRANCH][0], places=3,
                               msg='Scaling factor was supposed to be the {:.3f} changes per avg branch, got {:3f}'
                               .format(0.107, acr_result[CHANGES_PER_AVG_BRANCH][0]))

    def test_sf(self):
        self.assertAlmostEqual(3.841, acr_result[SCALING_FACTOR][0], places=3,
                               msg='Scaling factor was supposed to be the {:.3f}, got {:3f}'
                               .format(3.841, acr_result[SCALING_FACTOR][0]))

    def test_frequencies(self):
        for loc, expected_value in {'Africa': 0.082, 'Albania': 0.028, 'EastEurope': 0.081, 'Greece': 0.365,
                                    'WestEurope': 0.444}.items():
            value = acr_result[FREQUENCIES][0][np.where(acr_result[STATES][0] == loc)][0]
